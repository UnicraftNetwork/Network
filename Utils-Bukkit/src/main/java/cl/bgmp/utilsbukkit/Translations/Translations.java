...
public class Translations {
    ...
    private static final ImmutableSet<TranslatedLanguage> translatedLanguages = ImmutableSet.copyOf(new HashSet<TranslatedLanguage>() {{
    ...
    public static String get(@NotNull String key, @NotNull CommandSender sender, @Nullable String... args) {
        if (sender instanceof ConsoleCommandSender) return applyArgsToTranslation(templatePropertiesFile.getProperty(key));
        ...
        if (translatedLanguage == null) return applyArgsToTranslation(templatePropertiesFile.getProperty(key), args);
    ...
    private static TranslatedLanguage getTranslatedLanguageByLocale(final String locale) {
        return translatedLanguages.stream().filter(translatedLanguage -> translatedLanguage.getLocale().equals(locale)).findFirst().orElse(null);
    }

    private static Properties loadTemplatePropertiesFromResources() {
        final Properties properties = new Properties();
        try {
            properties.load(Translations.class.getClassLoader().getResourceAsStream("i18n/template/defaults.properties"));
            return properties;
        } catch (IOException e) {
            e.printStackTrace();
        }
