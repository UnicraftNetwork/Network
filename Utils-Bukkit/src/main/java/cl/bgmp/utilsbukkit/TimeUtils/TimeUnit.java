...
public enum TimeUnit {
    ...
    public static Time revertTimeFromMillis(long millisAmount) {
        if (millisAmount < TimeConstant.MILLIS_IN_SECOND.getValue()) return new Time(millisAmount, MILLISECONDS);

        final long secondsAmount = (long) (millisAmount / TimeConstant.MILLIS_IN_SECOND.getValue());
        if (secondsAmount < TimeConstant.SECONDS_IN_MINUTE.getValue()) return new Time(secondsAmount, SECONDS);

        final long minutesAmount = (long) (secondsAmount / TimeConstant.SECONDS_IN_MINUTE.getValue());
        if (minutesAmount < TimeConstant.MINUTES_IN_HOUR.getValue()) return new Time(minutesAmount, MINUTES);
        ...
        if (monthsAmount < TimeConstant.MONTHS_IN_YEAR.getValue()) return new Time((long) monthsAmount, MONTHS);
